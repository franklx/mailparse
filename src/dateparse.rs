...
pub fn dateparse(date: &str) -> Result<i64, MailParseError> {
    ...
    let state_vec =
        if date.contains(',')
            { vec![DateParseState::DayOfWeek, DateParseState::Date, DateParseState::Month, DateParseState::Year, DateParseState::Hour, DateParseState::Minute, DateParseState::Second, DateParseState::Timezone] }
        else
            { vec![DateParseState::DayOfWeek, DateParseState::Month, DateParseState::Date, DateParseState::Hour, DateParseState::Minute, DateParseState::Second, DateParseState::Timezone, DateParseState::Year] }
        ;
    let mut state_iter = state_vec.iter();
    let mut state = state_iter.next();
    for tok in date.split(|c| c == ' ' || c == ':') {
        if tok.is_empty() {
            continue;
        }
        match state {
            Some(DateParseState::DayOfWeek) => {
                state = state_iter.next();
                continue;
            }
            Some(DateParseState::Month) => {
